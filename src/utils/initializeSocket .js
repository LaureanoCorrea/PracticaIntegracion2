...
export function initializeSocket(httpServer) { //logica lado server
	...
	io.on('connection', (socket) => {
		...
		socket.on('deleteProductFromCart', async (productId) => {
			const productDeleted = await productsModel.findOneAndDelete(productId);
			const updatedCart = await productsModel.find();
			io.emit('cartUpdated', updatedCart);
		})

		socket.on('editProductQuantityInCart', async ({ productId, newQuantity }) => {
			const product = await productsModel.findOne(productId);
			const updatedCart = await productsModel.find();
			if (!product || !Number(newQuantity)) return;
